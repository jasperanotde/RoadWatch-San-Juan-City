...
<div data-te-modal-init class="bg-black bg-opacity-50 fixed left-0 top-0 z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none" data-te-modal-init id="editRoleModal{{ $role->id }}"  tabindex="-1" aria-hidden="true" role="dialog">
            ...
            <div class="relative flex-auto p-4">
                ...
                <div class="mx-10 mb-4">
                    ...
                    @foreach($permission as $value)
                        <label>{{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions[$role->id]) ? true : false, array('class' => 'name')) }}
                        {{ $value->name }}</label>
                    <br/>
                    @endforeach
                </div>
